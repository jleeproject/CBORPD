...
%                     clabel(C,h,'FontSize',fontsize)
                    ...
                    hold on;
                    plot(x2(imin),y2(imin),'ro', 'LineWidth',markerwidth)
                    colorbar()
                    if(create_pdf)
                        hold off;
                        colormap('gray');
                        fig.Position = [1, 50, 300 ,300];
...
%                         print(fig, sprintf('./figs/%s_o.pdf',regexprep( cell_prob_names{idx_function},'\s?\(.*\)\s?','' ) ), '-dpdf', '-bestfit')
                        mydrawpdf(fig, fontsize, sprintf('./figs/%s_o.pdf',regexprep( cell_prob_names{idx_function},'\s?\(.*\)\s?','' ) ))
...
%                     hatchfill(boundaries, 'single', -45, 3);


                    drawHatch(boundaries, ranges, ranges2, 'k', 'k', 0.3, 0, 'g')

%                     drawBoundaries(boundaries, ranges, ranges2, 'm', 'w', 0.3, 0.3)
                    colorbar()
                    disp('');
...
%                         print(fig, sprintf('./figs/%s_c.pdf',regexprep( cell_prob_names{idx_function},'\s?\(.*\)\s?','' ) ), '-dpdf', '-bestfit')
                        ...
                        mydrawpdf(fig, fontsize, sprintf('./figs/_%s_c.pdf',regexprep( cell_prob_names{idx_function},'\s?\(.*\)\s?','' ) ))
%                         print(fig, sprintf('./figs/%s_c.pdf',regexprep( cell_prob_names{idx_function},'\s?\(.*\)\s?','' ) ), '-dpdf', '-bestfit')
                        clf();cla();
%                     else
%                         title(cell_prob_names{idx_function})
