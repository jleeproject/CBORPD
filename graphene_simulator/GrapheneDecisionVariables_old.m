classdef GrapheneDecisionVariables_old < handle
...
%         Cb = [];
    ...
    methods
        function this = GrapheneDecisionVariables(T, L, W, t_top, t_back, Vg, Vds, Vb)
            if(nargin>7)
                setParameters(this, T, L, W, t_top, t_back, Vg, Vds, Vb);
            elseif(nargin>5)
                setParameters(this, T, L, W, t_top, t_back, Vg, Vds);
%                 this = setVoltages(this, Vg, Vds);
            elseif(nargin>0)
                setParameters(this, T, L, W, t_top, t_back);
            end
            init_domains(this);
        end
...
%             this.domains.L = [0.02 2].*1e-6; % : Impact of Channel Length and Width for Charge Transportation of Graphene Field Effect Transistor
            this.domains.L = [0.5 3].*1e-6; % : Impact of Channel Length and Width for Charge Transportation of Graphene Field Effect Transistor
            this.domains.W = [2 6].*1e-6;  %  2~5 : Impact of Channel Length and Width for Charge Transportation of Graphene Field Effect Transistor
            this.domains.t_top = [10 20].* 1e-9;
            this.domains.t_back = [10, 300].* 1e-9; %% https://www.sciencedirect.com/science/article/pii/S0375960121000037
%             this.domains.Vg = [-3 3];
...
%             this.domains.Vds = [-80 80];
        ...
        function out = getValues(this)
            out = [this.T; this.L; this.W; this.t_top; this.t_back; this.Vg; this.Vds; this.Vb ];
        end
        ...
        function this = setParameters(this, T, L, W, t_top, t_back, Vg, Vds, Vb)
            this.T = T;
            this.L = L;
            this.W = W;
            this.t_top = t_top;
