...
if (~exist('result','var') || sum([0,0]==size(result))>0);isFirstRun = true;disp('isFirstRun=true');end;    
if(isFirstRun)
%     warning('off','all')
...
%     load save/result_210317_random_ls;
    load results_210323_v1_1_1_Problems56_adj_4Acqs;
...
% 
%% Loop: Functions 
for idx_function=1:dim_func;
    %% Loop: Settings
    for idx_setting=1:num_settings
        setting = type_cell_settings{idx_setting};
        if(iscell(setting))
...
% %% ----------------------------------------------------------------------
        if(showDetail)
            fig1 = figure(50*(idx_setting-1)+idx_function);clf;
        end
%% Loop: Sample sizes
        for idx_samplesize=1:dim_cases_samplesize
            idx_pred_stt = 1;            idx_pred_end = 0;
            idx_feas_stt = 1;            idx_feas_end = 0;
            %% Loop: Repeats
            hasValue = false;
            for j=1:dim_repeats
                result = results_all{idx_samplesize,j,idx_function,idx_setting};
                if ([0,0]==size(result))
                    continue;
                end
                hasValue = true;
                
                results_all_backup = results_all;
                scr_setResultValuesToVariables
                preprocess_setAllfields
                yy_arr = result.y_history(1:n_init_sample);
                yy_cons_arr = result.y_history(1:n_init_sample);
                
                scr_setting_1_in_functions_sameObj
                scr_setting_3_in_settings
                results_all = results_all_backup;
...
%                 domainX = [sttX:budget]';
                ...
                % ----- Assign values --------------------------------------
                xxs_cell{idx_samplesize,idx_function,idx_setting}(idx_pred_stt:idx_pred_end)= ...
                cumsum(samplesize_history(n_init_sample+1:nEvaluations,1))  + sum(samplesize_history(1:n_init_sample));
            
                yys_cell{idx_samplesize,idx_function,idx_setting}(idx_pred_stt:idx_pred_end)= ...
    ...
    %                 cum_samplesize, feasible
                    feasible_cell{idx_samplesize,idx_function,idx_setting}(idx_pred_stt:idx_pred_end)  = feasible;
    ...
    %                 cum_samplesize(idxFeas), val(idxFeas)
                    ...
                    xxsFeasible_cell{idx_samplesize,idx_function,idx_setting}(idx_feas_stt:idx_feas_end) = cum_samplesize(idxFeas);
...
%                             title(sprintf('|f_{min} - min(mu)|: %.3g',diff_f_min(end)))
    ...
    %                         subp15.YLim(1) = 0;
                    ...
                    optFeasible_cell{idx_samplesize,idx_function,idx_setting}(idx_feas_stt:idx_feas_end)= val(idxFeas);
    ...
    %                 cum_samplesize, feasgap
                    feasgap_cell{idx_samplesize,idx_function,idx_setting}(idx_pred_stt:idx_pred_end)= feasgap;
    ...
    %                 cum_samplesize, pCons
                    probCons_cell{idx_samplesize,idx_function,idx_setting}(idx_pred_stt:idx_pred_end)= pCons;
                ...
                xxs_cell{idx_samplesize,idx_function,idx_setting} = xxs_cell{idx_samplesize,idx_function,idx_setting}(1:idx_pred_stt-1);
                yys_cell{idx_samplesize,idx_function,idx_setting} = yys_cell{idx_samplesize,idx_function,idx_setting}(1:idx_pred_stt-1);
                
                if(param_BO.hasConstraints)
                    feasible_cell{idx_samplesize,idx_function,idx_setting} = feasible_cell{idx_samplesize,idx_function,idx_setting}(1:idx_pred_stt-1);

                    xxsFeasible_cell{idx_samplesize,idx_function,idx_setting} = xxsFeasible_cell{idx_samplesize,idx_function,idx_setting}(1:idx_feas_stt-1);
                    optFeasible_cell{idx_samplesize,idx_function,idx_setting} = optFeasible_cell{idx_samplesize,idx_function,idx_setting}(1:idx_feas_stt-1);

                    feasgap_cell{idx_samplesize,idx_function,idx_setting} = feasgap_cell{idx_samplesize,idx_function,idx_setting}(1:idx_pred_stt-1);
                    probCons_cell{idx_samplesize,idx_function,idx_setting} = probCons_cell{idx_samplesize,idx_function,idx_setting}(1:idx_pred_stt-1);
                end

                ypred = smooth(xxs_cell{idx_samplesize,idx_function,idx_setting},yys_cell{idx_samplesize,idx_function,idx_setting},.1,'loess');
                ypred_cell{idx_samplesize,idx_function,idx_setting} = ypred;
               

                if(param_BO.hasConstraints)
                    feasible_pred_cell{idx_samplesize,idx_function,idx_setting} = smooth(xxs_cell{idx_samplesize,idx_function,idx_setting},feasible_cell{idx_samplesize,idx_function,idx_setting},.1,'loess');
    %                 xxsFeasible_pred_cell  {i,k,l} = smooth(xxs_cell{i,k,l},yys_cell{i,k,l},.1,'loess');
                    optFeasible_pred_cell{idx_samplesize,idx_function,idx_setting} = smooth(xxsFeasible_cell{idx_samplesize,idx_function,idx_setting},optFeasible_cell{idx_samplesize,idx_function,idx_setting},.1,'loess');
                    feasgap_pred_cell{idx_samplesize,idx_function,idx_setting} = smooth(xxs_cell{idx_samplesize,idx_function,idx_setting},feasgap_cell{idx_samplesize,idx_function,idx_setting},.1,'loess');
                    probCons_pred_cell{idx_samplesize,idx_function,idx_setting} = smooth(xxs_cell{idx_samplesize,idx_function,idx_setting},probCons_cell{idx_samplesize,idx_function,idx_setting},.1,'loess');
                end
...
%                     showTitles(objFunc, str_setting, k, l);
                    ...
                    if(param_BO.hasConstraints)
                        figure(11);
                        mysubplot(mysubplot_frame(1),mysubplot_frame(2),fnGetIdxOfSubplotWithRowCol(mysubplot_frame, idx_samplesize, fnGetIdxOfSubplotWithRowCol([dim_func,num_settings],idx_function,idx_setting)));
                        hold on;
                        % cum_samplesize =  cumsum(samplesize_history(n_init_sample+1:nEvaluations,1));
...
%% ----------------------------------------------------------------------
            ...
            [domainX, idxDomain] = sort(xxs_cell{idx_samplesize,idx_function,idx_setting});
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
            if(param_BO.hasConstraints)
                    % 2. optimal among feasible (val(idxFeas))
                figCon1 = figure(21);
                mysubplot(dim_func,num_settings,fnGetIdxOfSubplotWithRowCol([dim_func,num_settings], idx_function, idx_setting));
                ...
                hold on;
                [domainFeasX, idxFeasDomain] = sort(xxsFeasible_cell{idx_samplesize,idx_function,idx_setting});
                yPred = optFeasible_pred_cell{idx_samplesize,idx_function,idx_setting};
                showOneLegend( plot(domainFeasX, yPred(idxFeasDomain),marks{idx_samplesize}),arr_samplesize{idx_samplesize} )
                showTitles(objFunc, str_setting, idx_function, idx_setting);

                legend();
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
                    % 1. feasible
                figCon2 = figure(22);
                sp12 = mysubplot(dim_func,num_settings,fnGetIdxOfSubplotWithRowCol([dim_func,num_settings], idx_function, idx_setting));

                    hideLegend(plot([ sttX,budget]', log([0, 0]'),'k--' ));
                    hideLegend(plot([ sttX,budget]', log([1, 1]'),'k--' ));
                hold on;
                yPred = feasible_pred_cell{idx_samplesize,idx_function,idx_setting};
                showOneLegend( plot(domainX, yPred(idxDomain),marks{idx_samplesize}),arr_samplesize{idx_samplesize} )
                showTitles(objFunc, str_setting, idx_function, idx_setting);
    ...
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
                    % 3. feasiblity gap
                figCon3 = figure(23);
                sp13 = mysubplot(dim_func,num_settings,fnGetIdxOfSubplotWithRowCol([dim_func,num_settings], idx_function, idx_setting));

                hold on;
                yPred = feasgap_pred_cell{idx_samplesize,idx_function,idx_setting};
                showOneLegend( plot(domainX, yPred(idxDomain),marks{idx_samplesize}),arr_samplesize{idx_samplesize} )
                showTitles(objFunc, str_setting, idx_function, idx_setting);
    ...
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
                    % 4. probability of the constraints being satisfied
                figCon4 = figure(24);
                sp14 = mysubplot(dim_func,num_settings,fnGetIdxOfSubplotWithRowCol([dim_func,num_settings], idx_function, idx_setting));

                    hideLegend(plot([ sttX,budget]', log([0, 0]'),'k--' ));
                    hideLegend(plot([ sttX,budget]', log([1, 1]'),'k--' ));
                hold on;
                yPred = probCons_pred_cell{idx_samplesize,idx_function,idx_setting};
                showOneLegend( plot(domainX, yPred(idxDomain),marks{idx_samplesize}),arr_samplesize{idx_samplesize} )
                showTitles(objFunc, str_setting, idx_function, idx_setting);
                ...
                fig2 =figure(2);
                sp2 = mysubplot(dim_func,num_settings,fnGetIdxOfSubplotWithRowCol([dim_func,num_settings], idx_function, idx_setting));
...
%                 end
    %                 for i = 1:dim_cases_samplesize
                hold on;
                [domainX, idxDomain] = sort(xxs_cell{idx_samplesize,idx_function,idx_setting});
                yPred = ypred_cell{idx_samplesize,idx_function,idx_setting};
                showOneLegend( plot(domainX, yPred(idxDomain),marks{idx_samplesize}),arr_samplesize{idx_samplesize} )
                showTitles(objFunc, str_setting, idx_function, idx_setting);
...
%             
            figUncon2 =figure(4);
            mysubplot(dim_func,1,idx_function);
            if(show_exact_fmin)
                if(exist('objFunc','var')&& numel(objFunc)>0)
                    hideLegend(plot([ sttX,budget]', log([objFunc.optVal, objFunc.optVal]'),'k--' ));
                elseif(exist('opt_val_fn','var')&& numel(opt_val_fn)>0)
            ...
            if(param_BO.hasConstraints)
                [domainFeasX, idxFeasDomain] = sort(xxsFeasible_cell{idx_samplesize,idx_function,idx_setting});
                yPred = optFeasible_pred_cell{idx_samplesize,idx_function,idx_setting};
                showOneLegend( plot(domainFeasX, yPred(idxFeasDomain),marks{dim_cases_samplesize*(idx_setting-1)+idx_samplesize}),sprintf('%s:%s', arr_samplesize{idx_samplesize}, str_setting) )
            else
                [domainX, idxDomain] = sort(xxs_cell{idx_samplesize,idx_function,idx_setting});
                yPred = ypred_cell{idx_samplesize,idx_function,idx_setting};
                showOneLegend( plot(domainX, yPred(idxDomain),marks{dim_cases_samplesize*(idx_setting-1)+idx_samplesize}),sprintf('%s:%s', arr_samplesize{idx_samplesize}, str_setting) )
            end
%             hideLegend(plot([ sttX,budget]', log([objFunc.optVal, objFunc.optVal]'),'k--' ));
%                 for i = 1:dim_cases_samplesize
            hold on;
...
%             end
    ...
    % 
        ...
        if(showDetail)
            fig2 =figure(50*(idx_setting-1)+10+idx_function);clf;
            mysubplot(1,2,1);
            hideLegend(plot([ sttX,budget]', log([objFunc.optVal, objFunc.optVal]'),'k--' ));
            for idx_samplesize = 1:dim_cases_samplesize
                hold on;
                showOneLegend( plot(domainX, ypred_cell{idx_samplesize},marks{idx_samplesize}),arr_samplesize{idx_samplesize} )
            end
    %         title(sprintf('(%s,%s)',objFunc.name, str_setting ))
            legend()
            annotation(fig2,'textbox',...
    ...
    %         fig3 =figure(50*(l-1)+20+k);clf;
            mysubplot(1,2,2);
            hideLegend(plot([ sttX,budget]', log([objFunc.optVal, objFunc.optVal]'),'k--' ));
            for idx_samplesize = 2:dim_cases_samplesize
                hold on;
                res = mean(eval_f_at_xnew_history,2);
                yy = res(n_init_sample+1:nEvaluations);
                xx = cumsum(samplesize_history(n_init_sample+1:nEvaluations,1)) +  sum(samplesize_history(1:n_init_sample));
                showOneLegend( plot( xx,yy,marks{idx_samplesize}), arr_samplesize{idx_samplesize});
            end
            legend();
...
% end
    ...
    if(exist('figUncon2','var'));
        figUncon2.Name = 'Unconstrainted Optimization: Objective Value';
    end;
...
%             'String',{sprintf('%s','|f_{min} - min(mu)| (power = %.2g)', power )});
    ...
    strCols = cell(nCol,1);
    for idx_samplesize=1:nCol;
        strCols{idx_samplesize} = cell_obj_func{idx_samplesize}.name;
    end
%     for i=1:dim_cases_samplesize;
...
%         'String',{sprintf('GP: |f_{min} - min(mu)| (power = %.2g)', power )});
    ...
    strRows = cell(nRow,1);
    strCols = cell(nCol,1);
    for idx_samplesize=1:nRow;
        strRows{idx_samplesize} = cell_obj_func{idx_samplesize}.name;
    end
    for idx_samplesize=1:nCol;
        setting = type_cell_settings{idx_setting};
        if(iscell(setting))
        ...
        strCols{idx_samplesize} =  sprintf('%s\n',str_setting );
    end
%     strRows = {'r1','r2','r3','r4'};
%     strCols = {'c1','c2','c3'};
