...
%     zz2sq=reshape(gridCoord(:,2),dim,dim);
    ...
    hold on;
    scatter(x_history(1:nEvaluations+1,1), x_history(1:nEvaluations+1,2),10, 'k','filled','MarkerEdgeColor',[0 0 0]);
    ...
    colorbar();
    title(sprintf('fn:%s,n=%d', str_fn_name, sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))));
    xlabel('x');ylabel('y');
    ...
    hold on;
    scatter(x_history(1:nEvaluations+1,1), x_history(1:nEvaluations+1,2),10, 'k','filled');
    scatter(gridCoord(idx_min_mu,1), gridCoord(idx_min_mu,2),20,'d', 'LineWidth',1.5,'MarkerEdgeColor',[1 1 1]);
    scatter(gridCoord(maxIdx,1), gridCoord(maxIdx, 2), 20, 'k', 'LineWidth',1.5,'MarkerEdgeColor',[1 0 0]);
    colorbar();
    title('Mean');
...
%     hideLegend(    scatter(gridCoord(:,1), gridCoord(:,2),10, (log(fn_eval_true(gridCoord(:,1), gridCoord(:,2)))),'filled') ) ;
    hold on;
    hideLegend( scatter(x_history(1:nEvaluations+1,1), x_history(1:nEvaluations+1,2),10, 'k','filled','MarkerEdgeColor',[0 0 0]) );
...
%     hideLegend(    scatter(gridCoord(:,1), gridCoord(:,2),10, (log(fn_eval_true(gridCoord(:,1), gridCoord(:,2)))),'filled') ) ;
    ...
    legend('Location','best' )
    colorbar();
    title(sprintf('True fn:%s,n=%d', str_fn_name, sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))));
    xlabel('x');ylabel('y');
    % --------------------------------------------------------------------------------------
...
% ======================================================================================
    ...
    [mu_est, ~, sig_est] = funcHs(gridCoord);
...
%     idxFigGrid = 1;
    ...
    flippedImagesc(gridDomainD1, gridDomainD2, (reshape(acq_est, visualizer.getDim(), visualizer.getDim()))');
...
cum_samplesize =  cumsum(samplesize_history(n_init_sample+1:nEvaluations+1,1));
% if(visualizer.isShowAllGrid())
%     diff_f_min = (  eval_f_at_xnew_history(n_init_sample+1:nEvaluations+1,1)  - true_fmin );
%     idxNInf = find(eval_f_at_xnew_history(n_init_sample+1:nEvaluations+1,1) == -inf);
%     minDiff = min(diff_f_min(find(eval_f_at_xnew_history(n_init_sample+1:nEvaluations+1,1) > -inf)));
%     showOneLegend(    plot(  cum_samplesize, diff_f_min , 'r-', cum_samplesize(idxNInf),minDiff*ones(size(idxNInf)),'ro', [0 sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))], [0 0],'k--',  sum(samplesize_history(n_init_sample+1:nEvaluations+1,1)), abs(  eval_f_at_xnew_history(nEvaluations+1,1)  - true_fmin ), 'ob') ,'Diff(y)');
%     title(sprintf('|f_{min} - min(mu)|: %.3g',abs(  eval_f_at_xnew_history(nEvaluations+1,1)  - true_fmin )))
% else
%     f_x = eval_f_at_xnew_history(n_init_sample+1:nEvaluations+1,1);
%     showOneLegend(    plot(  cum_samplesize, f_x , 'r-', [0 sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))], [0 0],'k--',  sum(samplesize_history(n_init_sample+1:nEvaluations+1,1)),  eval_f_at_xnew_history(nEvaluations+1,1), 'ob') ,'Diff(y)');
%     title(sprintf('f_(x): %.3g',(  eval_f_at_xnew_history(nEvaluations+1,1)  )))
% end
    diff_f_min = (  eval_f_at_xnew_history(n_init_sample+1:nEvaluations+1,1)  - log(opt_val_fn ));
%     idxNInf = find(eval_f_at_xnew_history(n_init_sample+1:nEvaluations+1,1) == -inf);
%     minDiff = min(diff_f_min(find(eval_f_at_xnew_history(n_init_sample+1:nEvaluations+1,1) > -inf)));
%     showOneLegend(    plot(  cum_samplesize, diff_f_min , 'r-', cum_samplesize(idxNInf),minDiff*ones(size(idxNInf)),'ro', [0 sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))], [0 0],'k--',  sum(samplesize_history(n_init_sample+1:nEvaluations+1,1)), diff_f_min(end), 'ob') ,'Diff(y)');
    showOneLegend(    plot(  cum_samplesize, diff_f_min , 'r-',  [0 sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))], [0 0],'k--',  sum(samplesize_history(n_init_sample+1:nEvaluations+1,1)), diff_f_min(end), 'ob') ,'Diff(y)');
...
%     diff_x_f_min = vecnorm(xmin_history(n_init_sample+1:nEvaluations+1,:) - true_minX, 2, 2);
%     plot( cum_samplesize, diff_x_f_min, 'r-' , [0 sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))], [0 0],'k--',  sum(samplesize_history(n_init_sample+1:nEvaluations+1,1)), norm(xmin_history(nEvaluations+1,:) - true_minX, 2), 'ob');
%     title(sprintf('|x_{min} - argmin(mu)|: %.3g',sum(abs(xmin_history(nEvaluations+1,:) - true_minX))))
% end
    subplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigStatus, 2));
%     diff_x_f_min = vecnorm(xmin_history(n_init_sample+1:nEvaluations+1,:) - opt_sol_fn, 2, 2);
%     diff_x_f_min = min(vecnorm(xmin_history(n_init_sample+1:nEvaluations+1,:) - opt_sol_fn, 2, 2));
    diff_x_f_min = fn_get_minimum_distance(xmin_history(n_init_sample+1:nEvaluations+1,:), opt_sol_fn);
    plot( cum_samplesize, diff_x_f_min, 'r-' , [0 sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))], [0 0],'k--',  sum(samplesize_history(n_init_sample+1:nEvaluations+1,1)), diff_x_f_min(end), 'ob');
...
plot( cum_samplesize, samplesize_history(n_init_sample+1:nEvaluations+1,1), 'r-' , [0 sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))], [0 0],'k--',  sum(samplesize_history(n_init_sample+1:nEvaluations+1,1)), samplesize_history(nEvaluations+1,1), 'ob');
title(sprintf('Sample Size: %d (iter=%d)',samplesize_history(nEvaluations+1,1), iter))
...
% imagesc(gridDomainD1, gridDomainD2, (log(fn_eval_true(gridSquareD1, gridSquareD2)))');
% hold on;
% scatter(x_history(1:nEvaluations+1,1), x_history(1:nEvaluations+1,2),20, y_history(1:nEvaluations+1,1),'filled','MarkerEdgeColor',[0 0 0]);
...
% end
% subplot(2,2,3);
% plot(cumsum(samplesize_history(n_init_sample+1:nEvaluations+1,1)), samplesize_history(n_init_sample+1:nEvaluations+1,1), '-' , [0 sum(samplesize_history(n_init_sample+1:nEvaluations+1,1))], [0 0],'k--');
% title(sprintf('sample size=%d',samplesize_history(nEvaluations+1,1)));
% colorbar();
...
%                     History
%                     scatter(x_history(1:nEvaluations+1,1), x_history(1:nEvaluations+1,2),20, y_history(1:nEvaluations+1,1));
%                     scatter(x_history(1:nEvaluations+1,1), x_history(1:nEvaluations+1,2),10, 'k','filled');
% %                     Est MINIMUM POINT
% %                     scatter(gridCoord(idx_min_mu,1), gridCoord(idx_min_mu,2),20, fmin_history(nEvaluations+1+1,1), 'filled','d');
...
%     idxFigGrid = 1;
    idx_col = 3;
    subplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigGrid, idx_col));
    acq_est_grid = acqFunc.acquire(predictor, gridCoord, f_opt, iter);
