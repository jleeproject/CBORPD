...
if(param_BO.isInfillSearchGrid)
    ...
    % ----- Evaluated points ---------------------------------------------------------------------------------
        idxFig = 1;
        sp14 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFig, 4));
...
%         showOneLegend( scatter(queries_all(end,1), queries_all(end,2),20,'>', 'LineWidth',1.5,'MarkerEdgeColor',[0 .5 .5]) , str_last);
    ...
    % ======================================================================================
        % --------------------------------------------------------------------------------------
        idxFigGrid = 1;
        sp11 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigGrid, 1));
...
%         showOneLegend( scatter(queries_all(end,1), queries_all(end,2),20,'>', 'LineWidth',1.5,'MarkerEdgeColor',[0 .5 .5]) , str_last);
        ...
        % --------------------------------------------------------------------------------------
        idx_col = 3;
        sp13 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigGrid, idx_col));
...
%         showOneLegend( scatter(queries_all(end,1), queries_all(end,2),20,'>', 'LineWidth',1.5,'MarkerEdgeColor',[0 .5 .5]) , str_last);
        ...
        % --------------------------------------------------------------------------------------
        idx_col = 2;
        sp12 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigGrid, idx_col));
...
%         showOneLegend( scatter(queries_all(end,1), queries_all(end,2),20,'>', 'LineWidth',1.5,'MarkerEdgeColor',[0 .5 .5]) , str_last);
    ...
    % ======================================================================================
        ...
        idxFig = 2;
        sp24 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFig, 4));
...
%         showOneLegend( scatter(queries_all(idx_max_acq,1), queries_all(idx_max_acq, 2), 20, 'v', 'LineWidth',1.5,'MarkerEdgeColor',[1 0 0]) , str_max_acq);
    ...
    % FIGURE 2-1
    idx_col = 1;
    sp21 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigSearch, idx_col));
    hideLegend( scatter(x_history(1:nEvaluations,1), x_history(1:nEvaluations,2),10, 'k','filled','MarkerEdgeColor',[0 0 0]) );
    hold on;
...
%     showOneLegend( scatter(queries_all(idx_max_acq,1), queries_all(idx_max_acq, 2), 20, 'v', 'LineWidth',1.5,'MarkerEdgeColor',[1 0 0]) , str_max_acq);
    ...
    % FIGURE 2-2
    idx_col = 3;
    sp23 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigSearch, idx_col));
...
%     showOneLegend( scatter(queries_all(idx_max_acq,1), queries_all(idx_max_acq, 2), 20, 'v', 'LineWidth',1.5,'MarkerEdgeColor',[1 0 0]) , str_max_acq);
    ...
    % idxFigSearch = 2;
    sp22 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigSearch, idx_col));
...
idxFigStatus = 3;
sp31 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigStatus, 1));
cla;
...
xLimLeft = sum(samplesize_history(1:n_init_sample+1,1))-1;
...
sp31.XLim(1) = xLimLeft;
sp31.XLim(2) = xLimRight;


% opt_sol_fn
% opt_val_fn
if(~show_exact_fmin)
sp32 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigStatus, 2));
...
sp32.XLim(1) = xLimLeft;
sp32.XLim(2) = xLimRight;
sp32.YLim(1) = 0;
end


    idxFigStatus = 3;
sp33 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxFigStatus, 3));
...
sp33.XLim(1) = xLimLeft;
sp33.XLim(2) = xLimRight;
sp33.YLim(1) = 0;
...
%         meanFunc
        ...
        if(param_BO.isInfillSearchGrid)
            mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, 1, 5));
%             flippedImagesc(gridDomainD1, gridDomainD2, (reshape(acq, dim, dim))');
%             hold on;
%             scatter(x_history(1:nEvaluations,1), x_history(1:nEvaluations,2),10, 'k','filled');
%             scatter(gridCoord(idx_min_mu,1), gridCoord(idx_min_mu,2),20,'d', 'LineWidth',1.5,'MarkerEdgeColor',[1 1 1]);
...
%             title(sprintf('%s',param_BO.getStrTypeAcquisition()));
        ...
        % ----- Evaluated points ---------------------------------------------------------------------------------
            idxRow = 1;
            sp15 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, idxRow, 5));
            cla;
...
%             reshape(fn_true_mean(gridCoord(:,1), gridCoord(:,2)),visualizer.getDim,visualizer.getDim)'
    ...
    %         showOneLegend( scatter(queries_all(end,1), queries_all(end,2),20,'>', 'LineWidth',1.5,'MarkerEdgeColor',[0 .5 .5]) , str_last);
            ...
            colorbar();
            title(sprintf('Cons: fn:%s,%s', meanFunc.getName(), strCon  ));
            xlabel('x');ylabel('y');
            % --------------------------------------------------------------------------------------
            
            subplot(sp15); hold on;
            showBoundaries(boundaries_true, gridDomainD1, gridDomainD2, 'w', 'w', 0.5, 0.2)
            subplot(sp14); hold on;
            showBoundaries(boundaries_true, gridDomainD1, gridDomainD2, 'w', 'w', 0.5, 0.2)
        ...
        if(visualizer.isShowSearchTrails())
%             idxRow = 2;
            sp25 = mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, 2, 5)); cla;
%             fn_true_mean=meanFunc.getFnEval();
            hold on;
%             hideLegend( scatter(x_history(1:nEvaluations,1), x_history(1:nEvaluations,2),30, y_history(1:nEvaluations,:),'filled') );
%------------------------------------------------------------------------------------------------
...
%------------------------------------------------------------------------------------------------
            ...
            axis([dom(1,:), dom(2,:)]);
            colorbar();
            title(sprintf('True Cons: fn:%s,%s (u=%.3g)', meanFunc.getName(), strCon, urnd_feasiblity  ));
            xlabel('x');ylabel('y');

            subplot(sp25); hold on;
            showBoundaries(boundaries_true, gridDomainD1, gridDomainD2, 'w', 'w', 0.3, 0)
            showBoundaries(boundaries_ub, gridDomainD1, gridDomainD2, 'm', 'w', 0.3, 0)
            showBoundaries(boundaries_lb, gridDomainD1, gridDomainD2, 'm', 'w', 0.3, 0)

            if( type_infill_opt.strcmpi('DirectConstSampling'))
                [feas_eval, pCons] = funcFeas([gridCoord(:,1) gridCoord(:,2)], urnd_feasiblity);
                feasible = reshape(feas_eval,visualizer.getDim,visualizer.getDim)' ==-1;
                boundaries_est = bwboundaries(feasible);
                showBoundaries(boundaries_est, gridDomainD1, gridDomainD2);

                subplot(sp24);hold on;
                showBoundaries(boundaries_est, gridDomainD1, gridDomainD2);
                subplot(sp11);hold on;
                showBoundaries(boundaries_est, gridDomainD1, gridDomainD2);
                subplot(sp12);hold on;
                showBoundaries(boundaries_est, gridDomainD1, gridDomainD2);
                subplot(sp22);hold on;
                showBoundaries(boundaries_est, gridDomainD1, gridDomainD2);
                subplot(sp13);hold on;
                showBoundaries(boundaries_est, gridDomainD1, gridDomainD2);
                subplot(sp23);hold on;
                showBoundaries(boundaries_est, gridDomainD1, gridDomainD2);
                subplot(sp21);hold on;
                showBoundaries(boundaries_est, gridDomainD1, gridDomainD2);
            else
                    subplot(sp24);hold on;
                showBoundaries(boundaries_ub, gridDomainD1, gridDomainD2, 'r', 'w', 0.5, 0)
                showBoundaries(boundaries_lb, gridDomainD1, gridDomainD2, 'g', 'w', 0.7, 0)
                    subplot(sp11);hold on;
                showBoundaries(boundaries_ub, gridDomainD1, gridDomainD2, 'r', 'w', 0.5, 0)
                showBoundaries(boundaries_lb, gridDomainD1, gridDomainD2, 'g', 'w', 0.7, 0)
                    subplot(sp12);hold on;
...
%------------------------------------------------------------------------------------------------
            mysubplot(subplot_frame(1),subplot_frame(2),fnGetIdxOfSubplotWithRowCol(subplot_frame, 3, 5)); cla;
%             fn_true_mean=meanFunc.getFnEval();
            hold on;
