...
% %                     cell_feas_mat
% %                     cell_obj_mat
%                     data = cell_feas_mat{i,k,l};
%                     
%                     idxEndExp = max_rep_done(i,k,l);
%                     yyy =  sum(data, 2)./idxEndExp;
%                     yyy = yyy(n_init_sample:budget,:);
%                     showOneLegend( plot([n_init_sample:budget]', yyy , marks{dim_cases_samplesize*(l-1)+i}), sprintf('%s:%s', arr_samplesize{i}, str_setting));
                else
...
% %                     cell_feas_mat
% %                     cell_obj_mat
%                     data = cell_feasgap_mat{i,k,l};
%                     yyy =  median(data(:,1:idxEndExp),2);
%                     yyy = yyy(n_init_sample:budget);
% 
%                     yyp =  quantile(cell_feasgap_mat{i,k,l},.75,2);
%                     yyn =  quantile(cell_feasgap_mat{i,k,l},.25,2);
%                     yyp = yyp(n_init_sample:budget,:);
%                     yyn = yyn(n_init_sample:budget,:);
% %                     showOneLegend( plot([n_init_sample:budget]', yyy , marks{dim_cases_samplesize*(l-1)+i}), sprintf('%s:%s', arr_samplesize{i}, str_setting));
%                     if(show_errorbar)
%                         eb = errorbar([n_init_sample:budget]', yyy , yyn, yyp , marks{dim_cases_samplesize*(l-1)+i});
%                         eb.CapSize = 2;
%                     else
%                         eb = plot([n_init_sample:budget]', yyy , marks{dim_cases_samplesize*(l-1)+i});
%                     end
...
%                                 yout(:,j) = yyy(n_init_sample+1:floor(budget/samplesize));
%                                 if(floor(budget/samplesize)>size(fff,1) )
%                                     yfeas(1:size(fff,1)-n_init_sample,j) = fff(n_init_sample+1:size(fff,1));
%                                 else
%                                     yfeas(:,j) = fff(n_init_sample+1:floor(budget/samplesize));
%                                 end
        %                         for idx_value = n_init_sample+1:floor(budget/samplesize);
...
%                         yyy(idx_value-n_init_sample,1) = summed/num_feas;
...
%                     if(numel(yPred)>1)
                    ...
                    hold on;
                    showOneLegend(  plot(xxx_orig'*samplesize, nfeas./n_actually_repeated , marks{dim_cases_samplesize*(l-1)+i}), sprintf('%s:%s', arr_samplesize{i}, str_setting));
...
%                 if show_only_ours && contains(str_setting,'Testing9' ) 
%                     if numel(yPred)>0
%                         if ~exist('min_summary', 'var')
%                             min_summary = min(yPred);
%                         else
%                             min_summary = min(min_summary, min(yPred));
%                         end
%                         if ~exist('max_summary', 'var')
%                             max_summary = max(yPred);
%                         else
%                             max_summary = max(max_summary, max(yPred));
%                         end
%                         margin = (max_summary - min_summary)*.05;
%                         if numel(margin)>0
%                             if margin==0
%                                 margin = 1;
%                             end
%                             sp.YLim = [min_summary - margin, max_summary  + margin];
%                             prev_ylim = sp.YLim;
%                         end
%                     end
%                 elseif show_only_ours
%                     if exist('prev_ylim','var')
%                         sp.YLim = prev_ylim;
%                     end
% %                     if numel(margin)>0
% %                         if margin==0
