...
%                 domainX = [sttX:budget]';
                ...
                if(param_BO.hasConstraints)
                    ...
                    %% 1. feasible
                    [feasible, strCon] =  isFeasible(constraints.constraints{1}, @(x) optProb.conFunc{1}.evalWithVecX(x), xMuMinHistory(n_init_sample+1:nEvaluations,:));
                    ...
                    %% 3. feasiblity gap
                    feasgap = feasibilityGap(constraints.constraints{1}, @(x) optProb.conFunc{1}.evalWithVecX(x), xMuMinHistory(n_init_sample+1:nEvaluations,:));
          ...
          %% ----- LIBERALS --------------------------------------
                    if(hasLiberal )
                        %% 1. feasible
                        [feasible_liberal, strCon] =  isFeasible(constraints.constraints{1}, @(x)optProb.conFunc{1}.evalWithVecX(x) , xMuMinLiberalHistory(n_init_sample+1:nEvaluations,:));
                        ...
                        %% 3. feasiblity gap
                        feasgap_liberal = feasibilityGap(constraints.constraints{1}, @(x) optProb.conFunc{1}.evalWithVecX(x), xMuMinLiberalHistory(n_init_sample+1:nEvaluations,:));
