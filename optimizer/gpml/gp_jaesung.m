...
%     id = (nact+1):min(nact+nperbatch,ns);               % data points to process
    ...
    if strcmp(cstr,'covFITC') || strcmp(cstr,'apxSparse')    % cross-covariances
      Ks = feval(cov{:}, hyp.cov, x, xs); Ks = Ks(nz,:); % res indep. of x
    else
    ...
    fmu = sum(Fmu,2)/N;                                   % predictive means
    % == [IMPORTANT] PREDICTIVE VARIANCE MATRIX =============
...
%       fs2 = kss + sum(Ks.*LKs,1)';                    % predictive variances
      fs2 = kss - Ks'**Ks
    end
    ...
    if(strcmp(func2str(lik{:}), 'likGauss_taylored_log_sample_std'))
%         Added by Jaesung
        if nargin<9
          [Lp, Ymu, Ys2] = feval(lik{:},hyp.lik,[],   Fmu(:),Fs2(:), '', xs);
        else
          Ys = repmat(ys(id),1,N);
          [Lp, Ymu, Ys2] = feval(lik{:},hyp.lik,Ys(:),Fmu(:),Fs2(:), '', xs);
        end
    else
%         Existing
        if nargin<9
...
%     fs2(id) = max(fs2(id),0);   % remove numerical noise i.e. negative variances
%     Fs2 = repmat(fs2(id),1,N);     % we have multiple values in case of sampling
%     if(strcmp(func2str(lik{:}), 'likGauss_taylored_sample_size'))
% %         Added by Jaesung
%         if nargin<9
%           [Lp, Ymu, Ys2] = feval(lik{:},hyp.lik,[],   Fmu(:),Fs2(:), '', xs);
%         else
