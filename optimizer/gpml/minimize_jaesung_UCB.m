function [X, fX, i] = minimize_jaesung_UCB(X, f, length, varargin)
...
hasSamplesize=false;
if isfield(X,'samplesize')
    hasSamplesize = true;
    samplesize = getfield(X,'samplesize');
    X = rmfield(X,'samplesize');
end
mm = getfield(X,'mean');
X = rmfield(X,'mean');
...
[f0 df0] = feval(f, setfield(setfield(X,'samplesize',samplesize),'mean',mm), varargin{:});          % get function value and gradient
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        ...
        % JL: IF hyp has samplesize
        if(hasSamplesize)
            [f3 df3] = feval(f, setfield(setfield(rewrap(Z,X+x3*s),'samplesize',samplesize),'mean',mm), varargin{:});
        else
            [f3 df3] = feval(f, setfield(rewrap(Z,X+x3*s),'mean',mm), varargin{:});
        end
        df3 = unwrap(df3);
