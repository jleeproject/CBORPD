function [X, fX, i] = minimize_jaesung(X, f, length, varargin)
...
[f0 df0] = feval(f, setfield(X,'samplesize',samplesize), varargin{:});          % get function value and gradient
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        ...
        if(hasSamplesize)
            [f3 df3] = feval(f, setfield(rewrap(Z,X+x3*s),'samplesize',samplesize), varargin{:});
        else
