function [ret_minval,final_xatmin,history, queries, queryVals, queryVals2, queryVals3, qeuryValCon] = diRect_nargout_func_3_const...
...
getopts(opts, ...
 ...
 'showits',    0,...          % print iteration stats
...
[thirds , lengths, c , fc, con, feas_flags minval,xatmin,perror,...
        history,szes,fcncounter,calltype, queries, queryVals, queryVals2, queryVals3, qeuryValCon] =...
...
while perror > tol
   ...
   %-- and divide -------------------------------------------------%
   for i = 1:size(S,2)
      [lengths,fc,c,con,feas_flags,szes,fcncounter,success, queries, queryVals, queryVals2, queryVals3] = ...
          DIRdivide(bounds(:,1),bounds(:,2),Problem,S(1,i),thirds,lengths,...
          fc,c,con,feas_flags,fcncounter,szes,impcons,calltype,...
          maxEvals, queries, queryVals, queryVals2, queryVals3, varargin{:});
   end
...
function [l_thirds,l_lengths,l_c,l_fc,l_con, l_feas_flags, minval,xatmin,perror,...
        history,szes,fcncounter,calltype, queries, queryVals, queryVals2, queryVals3, queryValCon] = ...
...
[l_fc(1),l_con(1), l_feas_flags(1), point, l_fc2(1), l_fc3(1), c_fc] = ...
  ...
  queryVals2 = zeros(queryStoreSize, 1);
  queryVals3 = zeros(queryStoreSize, 1);
  queries(1,:) = point';
  queryVals(1) = l_fc(1);
  queryVals2(1) = l_fc2(1);
  queryVals3(1) = l_fc3(1);
  queryVals3(1) = l_fc3(1);
  queryValCon = c_fc;
  numQueries = 1;
fcncounter = 1;
...
%------------------------------------------------------------------%
function [lengths,fc,c,con,feas_flags,szes,fcncounter,pass, queries, queryVals, queryVals2, queryVals3] = ...
    ...
    queryVals, queryVals2, queryVals3, varargin)
...
f_left2     = zeros(1,lssize);
f_right2    = zeros(1,lssize);
f_left3     = zeros(1,lssize);
f_right3    = zeros(1,lssize);
...
%     fcncounter,
    break; % Don't go beyond maxEvals
      ...
      [f_left(i), con_left(i), fflag_left(i), pointLeft, f_left2(i), f_left3(i)]    = ...
        CallObjFcn(Problem,newc_left(:,i),a,b,impcons,calltype,varargin{:});
      [f_right(i), con_right(i), fflag_right(i), pointRight, f_right2(i), f_right3(i)] = ...
    ...
    if fcncounter < queryStoreSize 
      ...
      queryVals2(fcncounter+1) = f_left2(i);
      queryVals2(fcncounter+2) = f_right2(i);
      queryVals3(fcncounter+1) = f_left3(i);
      queryVals3(fcncounter+2) = f_right3(i);
    end
...
%------------------------------------------------------------------%
function [ret_value, con_eval_values] = CallConstraints(Problem,x,a,b,varargin)
...
if isfield(Problem,'constraint')
    if ~isempty(Problem.constraint)
        con_eval_values = zeros(1,Problem.numconstraints);
...
%                 if double(Problem.constraint(i).func) == double(Problem.f)                    
                if strcmp( func2str(Problem.constraint(1).func), func2str(Problem.f))
...
%------------------------------------------------------------------%
function [fcn_value, con_value, feas_flag, point, fcn_value2, fcn_value3, con_eval_values] = ...
...
if calltype == 1
    ...
    [fcn_value, fcn_value2, fcn_value3] = feval(Problem.f,point,varargin{:});
%     fcn_value = Problem.f(point);
    con_eval_values = [];
elseif calltype == 2
    %-- f returns all constraints
    [fcn_value, fcn_value2, fcn_value3, cons] = feval(Problem.f,point,varargin{:});
    for i = 1:length(cons)
        if cons > 0
...
elseif calltype == 3
    ...
    [fcn_value, fcn_value2, fcn_value3] = feval(Problem.f,point,varargin{:});
    [con_value, con_eval_values] = CallConstraints(Problem,x,a,b,varargin{:});
elseif calltype == 4
    %-- f returns feas flag
...
%             if strcmp( class(Problem.constraint(1).func), class(Problem.f))
            if strcmp( func2str(Problem.constraint(1).func), func2str(Problem.f))
