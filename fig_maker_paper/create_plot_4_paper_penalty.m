...
%                 end
                ...
                if(isfirst)
                    ...
                    names_methods = {num2str(setting.type_cell_settings{1}{2}), ...
                        num2str(setting.type_cell_settings{2}{2}), ...
                        num2str(setting.type_cell_settings{3}{2}), ...
                        num2str(setting.type_cell_settings{4}{2}), ...
                        };
            ...
            if(pred_method == MEDIAN)
                if(showOnlyFeasible)
                    ...
                    if(show_errorbar)
                        ...
                        %% FIGURE 1: 
%                         zzz= smooth([xxx]'*samplesize , yyy,.2,'loess')
                        if contains(class(param_BO.objFunc),'Graphene')
                            zzz = movmedian(yyy,50);                        
                        else
                            zzz=yyy; 
                        end
                        eb = myplot([xxx]'*samplesize, zzz, marks{dim_cases_samplesize*(l-1)+i});
                    end
%                     showOneLegend(eb, sprintf('%s:%s', arr_samplesize{i}, str_setting));
                    ...
                    eb = myplot(xxx_orig'*samplesize, zzz , marks{dim_cases_samplesize*(l-1)+i});
                    showOneLegend(eb, sprintf('%s', names_methods{l}));
