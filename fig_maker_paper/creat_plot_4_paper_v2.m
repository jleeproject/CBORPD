...
%     mysubplot = @(m,n,p) figure(p);
    myfigure = @(x1,x2,x3) figure(plot_indice(mod(x3-1, plot_dim_func)+1, [.06, 0.02]);
    mysubplot = @(m,n,p) subtightplot(1,1,1);
else
    mysubplot = @(x1,x2,x3) subtightplot(plot_dim_func,1,plot_indice(mod(x3-1, plot_dim_func)+1), [.06, 0.02]);
...
%                     showOneLegend(eb, sprintf('%s:%s', arr_samplesize{i}, str_setting));
                    ...
                    if(create_pdf)
                        hold off;
%                         colormap('gray');
%                         figSummary.Position = [1, 50, 400 ,400];
%                         setFontSize(figSummary,fontsize);
%                         saveas(sp, sprintf('./figs/%s_o.pdf',strrep( cell_prob_names{idx_function},':','-' ) ) , 'pdf'  );
%                         print(figSummary, sprintf('./figs/%s_o.pdf',regexprep( cell_prob_names{idx_function},'\s?\(.*\)\s?','' ) ), '-dpdf', '-bestfit')
                        mydrawpdf(figSummary, fontsize, sprintf('./figs/%s_fo.pdf',regexprep( cell_prob_names{idx_function},'\s?\(.*\)\s?','' ) ))
                        clf();cla();
...
figSummary = figure(idxSummary);
annotateTextTitle(figSummary, strrep(A_RUN_PURPOSE,'_',' '));
